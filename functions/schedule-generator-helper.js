...
function attachSessionAndSpeakersTogether(session, speakersRaw) {
  if (isDefined(session.speakers)) {
    ...
    session.speakers = session.speakers.filter(speaker => isDefined(speaker));

    for (let speakerIdx = 0; speakerIdx < session.speakers.length; speakerIdx++) {
      let speaker = speakersRaw.find(speaker => speaker.id === session.speakers[speakerIdx])

      if (isDefined(session.speakers[speakerIdx])
        && !isDefined(session.speakers[speakerIdx].id)
        && isDefined(speaker)) {
        session.speakers[speakerIdx] = speaker;
        let tempSession = clone(session);
...
function generateSchedule(scheduleDB, sessionsDB, speakersDB) {
  ...
  if (hasSpeakersAndSessionsAndSchedule(speakers, sessions, schedule.days)) {
    schedule.tags = [];
    let dayIdx = 0, scheduleLen = schedule.days.length;

    for (; dayIdx < scheduleLen; dayIdx++) {
      let day = schedule.days[dayIdx];
      day.tags = [];

      let timeSlotIdx = 0, timeslotsLen = day.timeslots.length;

      for (; timeSlotIdx < timeslotsLen; timeSlotIdx++) {
        let timeslot = day.timeslots[timeSlotIdx];
        let sessionIndex = 0, sessionsLen = timeslot.sessions.length;

        for (; sessionIndex < sessionsLen; sessionIndex++) {
          let subSessIdx = 0, subSessionsLen = timeslot.sessions[sessionIndex].length;

          for (; subSessIdx < subSessionsLen; subSessIdx++) {
            let session = sessions[timeslot.sessions[sessionIndex][subSessIdx]];

 ...
 */
export const newInvoiceRequest = functions.firestore
  ...
  .onCreate(async (snap, context) => {
    ...
    return helpers.sendInfoIntoSlack({
      text: 'Faktura vygenerována',
    });
  });

export const newInvoiceSendInvoice = functions.firestore
  .document('invoiceRequests/{requestId}')
  .onUpdate(async (change, context) => {
    if (
      change.after.data().invoiceFound === true &&
      change.after.data().invoiceSended === false
    ) {
      const downloaded = await fakturoid.downloadInvoiceById(
        change.after.data().invoiceId
      );
      const mail = await sendgrid.sendInvoiceInEmail(
        downloaded,
...
export const invoicePaid = functions.https.onRequest((req, res) => {
  ...
  if (key !== functions.config().fakturoid.key) {
    ...
    if (body.status === 'paid' && body.event_name === 'invoice_paid') {
      return sendDiscountCodeByInvoiceId(invoiceId)
        .then(() => {
          return helpers.sendInfoIntoSlack({
            text:
              'Faktura zaplacena, slevový kupón vygenerován a zaslán na email :moneybag:',
          });
        })
        .then(() => {
          return res.send(200);
...
async function sendDiscountCodeByInvoiceId(invoiceId): Promise<Object> {
  ...
  if (snapshot.empty) {
    ...
    const code = await tito.generateTitoCode(
      info.companyName,
      invoiceId,
      info.countTicketsNormal === null ? 0 : info.countTicketsNormal,
      info.countTicketsVIP === null ? 0 : info.countTicketsVIP
    );
    await admin
      .firestore()
      .collection('invoiceRequests')
    ...
    return sendgrid.sendDiscountCode(
      code,
      'https://ti.to/devfest-cz/' +
        new Date().getFullYear() +
        '/discount/' +
        querystring.escape(code),
      info.email
    );
  }
}
 ...
 * @param countVIP
 */
async function informIntoSlackAboutNewInvoice(
  company: string,
  countNormal: number,
  countVIP: number
) {

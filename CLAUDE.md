...
| `npm run a11y`    | Mock-data build into `dist-a11y/` + axe accessibility audit (`scripts/a11y.mjs`, `@axe-core/playwright`) |
...
`DEVFEST_LIVE_API=1 npm run dev` proxies `/api/*` to the deployed functions instead.

No lint script is configured — TypeScript strict mode provides type safety. The only automated check is `npm run a11y` (axe against an `A11Y_MOCK=1` build in `dist-a11y/`; see "Browser data access").
...
**Every `ld+json` block goes through `ldJson()` (`src/lib/json-ld.ts`) — never `JSON.stringify` directly.** `set:html` writes its string verbatim and `JSON.stringify` does not escape `<`, so one `</script>` inside any interpolated value ends the script element early: the structured data is destroyed and everything after it becomes live markup. That is not theoretical — the lineup graphs interpolate Sessionize free text (bios, taglines, talk titles, abstracts, room names) that speakers write themselves, and the FAQ answers contain inline HTML by design. `ldJson` escapes `<` as `\u003c`, which is valid JSON and parses back identically. Apply it at *every* emit site, including ones whose data is all in-repo constants today.

**One entity graph, not a pile of objects.** Every fact about the event, the venue, the organisation and the ticket offers lives in **`src/lib/event.ts`**, which mints each node at a stable `@id` (`…/#event`, `…/#organization`, `…/#gug`, `…/#website`, `…/#venue`). Pages emit a single `@graph` and reference those nodes with `{'@id': …}` instead of restating them — a subpage that re-describes the event describes a *second* event as far as a crawler is concerned.

Two rules that follow from that, both of which were bugs first:
- **A referenced `@id` must be DEFINED in the same document.** `@id` resolves within a document, not across a site. `WebPage.about`, a speaker's `performerIn` and a talk's `superEvent` all reference `#event`, so non-home pages emit `eventStubSchema` — a minimal Event carrying the venue, and nothing (no offers, description or image) that would make a subpage compete with `/` for *being* the event.
- **`Event.organizer` is GUG.cz (`#gug`), not the DevFest.cz brand node.** GUG.cz, z.s. is the legal organiser and the visible page says so; structured data that disagrees with the page is worse than none. DevFest.cz (`#organization`) is the brand: `WebSite.publisher`, `worksFor` for the crew, `parentOrganization` → `#gug`.

**The lineup pages are pre-rendered, and that is an SEO fix, not a perf one.** `/speakers`, `/sessions` and `/agenda` are the pages people search for by name (a speaker, a talk title), and their HTML used to be a heading plus "Loading". `src/lib/lineup-build.ts` reads the same `/api/lineup` endpoint **at build time**, memoised across the three pages, and hands the result to the islands as `initialSpeakers` / `initialSessions`; Astro server-renders `client:load` components, so the names land in the static document. The island's own mount fetch still runs and replaces the snapshot, so a visitor always sees the live roster.

Everything that constrains it:
- **A failed refetch must never delete the pre-render.** The whole seed-refetch-degrade lifecycle lives in ONE hook, `usePrerenderedFetch` (`src/components/usePrerenderedFetch.ts`) — it was three hand-rolled copies and they drifted into bugs the others did not have. It degrades to the error state only when nothing is already rendered. Without that guard a transient `/api/lineup` 503 wipes a fully-painted grid a moment after it appears — and hands a JS-executing crawler an error string in place of the content the HTML just gave it. A stale snapshot beats an error banner. (A healthy endpoint returning an *empty* roster is different: that is authoritative live data and does override the snapshot.)
- **`robots.txt` must not disallow `/api/`.** Same reason: Google renders JS, a disallowed URL is never fetched during rendering, and the rendered DOM is what gets indexed.
- **Never shuffle inside a pre-rendered island.** `Math.random()` in the server-render gives the client a different order and React throws the tree away. `/sessions` still rotates so no track or room keeps the top of the grid — but the PAGE does it, once per build (and the hosting workflow rebuilds daily), and the island re-shuffles only when the refetched roster is not the one already on screen (`order` on the hook may REORDER the fresh items, never substitute the old ones — returning the previous array would pin the page to the build-time snapshot and drop the room or slot change the refetch exists to deliver). Re-shuffling an identical, visible grid is a content jump on that page's main list, right when someone is reaching for a card. (`SpeakersTeaser` on `/` is left un-pre-rendered — it shuffles per load, and the home page is content-rich already.)
- **Only pass data the server-render actually uses.** `/sessions` and `/agenda` take `initialSessions` and *not* `initialSpeakers`: nothing in their SSR'd markup reads the speaker map (cards render each session's embedded `speakers[]` refs), and seeding it serialised ~42 KB of bios and links per page into the props blob for a lookup only `SessionDetail` performs — which `speakerFromRef` already covers until the fetch lands.
- **`/agenda` only pre-renders once something is scheduled.** With no `startsAt` anywhere, the island's `dayRange()` returns null and it renders its "schedule lands closer to the event" state, so handing it the programme serialises ~80 KB to produce a 162-byte empty state. `hasSchedule` gates both the island's props and the `<noscript>` note.
- **The snapshot is a deploy-time copy, and the deploy is scheduled.** Sessionize syncs into Firestore at 06:00 Prague daily; a static file that nobody rebuilt returns the same stale bytes however often it is re-crawled, so `firebase-hosting-merge.yml` also runs on a daily cron (07:00 UTC) to redeploy the live commit with fresh data. Without it a withdrawn speaker stays in the published `ItemList` until someone happens to merge something. GitHub disables a cron after 60 days of repository inactivity, which is worth knowing because the quiet months are exactly when it is the only thing correcting the published data. Only those three pages carry a `lastmod` in the sitemap for the same reason — they are the only URLs whose bytes a rebuild changes, and stamping all twelve with the build instant spends the signal on pages that never change.
- **A failed build-time read is loud and, on the production deploy, fatal.** Falling back to empty silently is a green deploy that drops every speaker and talk from the HTML *and* deletes the `ItemList` graphs — the symptom Search Console reports as items disappearing. It retries on the same schedule as `functions/src/lib/http.ts` (3 attempts, 1s/2s, never on a 4xx), prints a banner, and throws when `process.env.CI` is set. Two deliberate exceptions: the **PR preview** workflow sets `LINEUP_BUILD_OPTIONAL=1` — a throwaway channel turning every open PR red over an endpoint blip protects nothing — and the merge workflow takes a `skip_lineup` **`workflow_dispatch` input**, because an override you cannot reach from the Actions UI is not an override. A skipped read in CI also emits a `::warning::` annotation, so a green run does not hide it. The only PR check, `npm run a11y`, runs on fixtures and can never catch this.
- **The dev/fixture switch is `__DEVFEST_API_FIXTURES__` — never `import.meta.env.DEV` and never `NODE_ENV`.** Both of those resolve from `process.env.NODE_ENV` (Vite sets `DEV` from it even for a build), so either one lets a shell that exports `NODE_ENV=development` publish `scripts/a11y-mocks` fixtures as the real lineup, in the HTML and the JSON-LD, exit 0. The flag is a `define` handed down by the `apply: 'serve'` dev-server plugin in `astro.config.mjs`, which cannot run during a build, so it cannot be true during one. Locally and under `A11Y_MOCK=1` the build-time read comes from the same fixtures the dev server answers `/api/*` with, so a pre-render can never disagree with what the island then fetches. `DEVFEST_LIVE_API=1` turns the fixtures off **and** proxies `/api/*` to the deployed site (a dev server has no Hosting rewrite, so without the proxy the flag would just 404 the island); `LINEUP_BUILD_ENDPOINT` overrides the build-time URL.
- **`npm run a11y` builds to `dist-a11y`, not `dist`.** `firebase.json` deploys `dist`, and an `A11Y_MOCK=1` build now fills the lineup pages with fixture people — so sharing the directory made a local `npm run a11y` followed by a hosting deploy publish Ada Lovelace as the speaker roster, exit 0. Any fixture-backed *build* also prints a banner saying its output must not be deployed. Keep the `--outDir` in `package.json` and the `DIST` constant in `scripts/a11y.mjs` in step.

`src/lib/lineup-schema.ts` builds the page-level graphs from that same data: an `ItemList` of `Person` for `/speakers`, and an `ItemList` of talks for `/sessions` (unordered — the list is shuffled) and `/agenda` (time-ordered). Three things it gets right that are easy to get wrong again:
- **A talk becomes a schema.org `Event` only once it is actually scheduled**, which is `isScheduled()` (`src/lib/agenda.ts`), not a truthiness check on `startsAt`. Google requires `startDate`, and synthesising one from the conference's own start would be a fabricated fact — so an unscheduled talk is a plain `ListItem` carrying its name. The predicate matters because a bare date (`2026-10-30`, no time) is truthy but unplaceable: the grid would treat that talk as unscheduled while the graph published it as a dated Event. One predicate, so the page and the graph cannot disagree.
- **`startDate`/`endDate` are made absolute with `toAbsoluteIso` (`src/lib/agenda.ts`).** Sessionize emits event-LOCAL strings with no offset. The timetable deliberately never resolves those to an instant, but schema.org dates *are* instants, so publishing the naive string makes any UTC-defaulting consumer read the whole programme an hour off from the conference node — which is offset-qualified. The offset is derived from the date, not hardcoded (CET vs CEST).
- **The `/agenda` graph is filtered with `isDisplayableSession`, not `isAgendaSession`.** The grid keeps lunch, breaks and the emcee's `HOST` rows because it draws them as bands; publishing them as programme entries states that "HOST" is a talk at this conference.
- **Every `Event` node — the conference AND each talk — names `#gug` as `organizer`.** A talk pointing at the DevFest.cz brand node instead says each talk is run by a different organisation than the conference it belongs to.

`/team` emits its crew the same way (`ItemList` of `Person`, tied to the Organization node) from the `team` content collection — it does not read the lineup.

`robots.txt` allows everything public and disallows nothing.

...
- Visibility is enforced **at write time** in `refresh-cache.ts` via `isWebsiteVisible()` (`functions/src/tickets/tito-api.ts`): keep only releases where `state ∈ {live, on_sale}` AND `accessibility ∈ {public, undefined}`. Drafts/archived/private/protected releases never land in `/tickets` (the node is publicly readable, so they must not be cached at all). The browser's `filterDisplayable` (`src/lib/tito.ts`) mirrors the rule as defence-in-depth.
- Any `sale_status` that survives the filter is rendered with a status badge (on sale / sold out / paused / coming soon / ended). Buy CTA enabled only when `purchasable` (`releaseStatus()`). Buy URL pattern: `https://ti.to/<account>/<event>/with/<release-slug>`.

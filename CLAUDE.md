...
- **Deploy ordering:** the `firebase.json` rewrites (deployed by hosting-merge) point at functions deployed by functions-merge; a first deploy can briefly 404 `/api/*` until the function lands (graceful — the island shows its unavailable state, then a reload recovers). Preview channels pin the **production** function (`pinTag`).

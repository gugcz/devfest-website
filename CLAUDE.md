...
- **The snapshot is a deploy-time copy, and the deploy is scheduled.** Sessionize syncs into Firestore at 06:00 Prague daily; a static file that nobody rebuilt returns the same stale bytes however often it is re-crawled, so `firebase-hosting-merge.yml` also runs on a daily cron (07:00 UTC) to redeploy the live commit with fresh data. Without it a withdrawn speaker stays in the published `ItemList` until someone happens to merge something.
- **A failed build-time read is loud and, in CI, fatal.** It used to warn and fall back to empty, which is a green deploy that silently drops every speaker and talk from the HTML *and* deletes the `ItemList` graphs — the exact symptom Search Console reports as items disappearing. It now retries once, prints a banner, and throws when `process.env.CI` is set. `LINEUP_BUILD_OPTIONAL=1` ships anyway (an urgent hosting deploy while `/api/lineup` is down is a legitimate thing to want). The only PR check, `npm run a11y`, runs on fixtures and can never catch this.
- **The dev/fixture switch is `import.meta.env.DEV` only — do not widen it to `NODE_ENV`.** That is ambient shell state, so a plain `astro build` from a dev container would publish `scripts/a11y-mocks` fixtures as the real lineup, in both the HTML and the JSON-LD, exit 0. Locally and under `A11Y_MOCK=1` the build-time read comes from the same fixtures the dev server answers `/api/*` with, so a pre-render can never disagree with what the island then fetches. `DEVFEST_LIVE_API=1` hits the deployed function; `LINEUP_BUILD_ENDPOINT` overrides the URL.
...
- **A talk becomes a schema.org `Event` only once it has a real `startsAt`.** Google requires `startDate`, and synthesising one from the conference's own start would be a fabricated fact — so an unscheduled talk is a plain `ListItem` carrying its name.

...
- Visibility is enforced **at write time** in `refresh-cache.ts` via `isWebsiteVisible()` (`functions/src/tickets/tito-api.ts`). Drop: `archived`, `secret`, `expired`, `upcoming`, `off_sale` (paused), `locked`. Keep: on-sale releases and sold-out releases (sold-out shows visitors that a tier ran out). Drops happen before write so unpublished release data never lands in the publicly readable `/tickets` node. The browser's `filterDisplayable` (`src/lib/tito.ts`) mirrors the rule as defence-in-depth.

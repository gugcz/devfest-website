 ...
 *
 * Steps: firebase.json matches dist → direct-load every route → click through
 * every route from `/` without a reload → accept the cookie banner. Any
 * `securitypolicyviolation` or enforced CSP console error fails the run.
...
import { computePolicy, currentPolicy } from './csp-header.mjs';
...
async function run() {
	step = 'static check';
	const { policy } = await computePolicy();
	const { value: header } = await currentPolicy();
	if (header !== policy) {
		console.error('firebase.json CSP header is stale. Run `npm run build` and commit firebase.json.');
		process.exit(1);
	}

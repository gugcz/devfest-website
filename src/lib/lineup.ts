...
export function fetchLineup(signal?: AbortSignal): Promise<Lineup> {
	return fetchParsed(isDisplayableSession, signal);
}

/**
 * Same `/api/lineup` read as {@link fetchLineup}, but for the `/agenda`
 * timetable: `isAgendaSession` KEEPS service + plenum sessions (breaks / lunch /
 * registration / keynote) so they can render as full-width bands, where
 * `/sessions` hides them.
 */

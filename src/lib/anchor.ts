...
function hold(el: HTMLElement): void {
	...
	const tick = () => {
		...
		if (y === lastY) {
		...
		} else if (docTop !== lastDocTop && !Number.isNaN(lastDocTop)) {
			...
			// cutting it off: `scrollIntoView` with no `behavior` follows the CSS
...
export function keepAnchorLanded(): void {
	...
	if (el && !scrollWasRestored(el)) hold(el);
	...
	// Deliberately not gated on `event.defaultPrevented`: ClientRouter's own
	// document click handler runs first and cancels same-page hash links so it
	// can scroll them itself, which is precisely the jump this has to hold —
	// checking for it skipped every in-page click on the site.

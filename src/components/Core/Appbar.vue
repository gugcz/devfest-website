<template>
  ...
  >
    ...
    ></v-app-bar-nav-icon> -->
    <v-toolbar-title class="google-font px-0 mr-1" style="width: 220px"
      ><router-link
        to="/"
        class="google-font grey--text text--darken-2"
        style="text-decoration: none; font-size: 110%;font-weight:400"
      >
        <v-img contain :src="require('@/assets/img/devfest.svg')" width="150px"></v-img>
    ...
    <!-- <v-btn v-if="!userFound && !loadingUser" to="/profile" style="text-transform: capitalize;" rounded depressed class="mx-2 google-font hidden-sm-and-down" color="#4285f4" dark> DevFest Profile -->
    <!-- </v-btn> -->
    <!-- 
...
export default {
  name: "AppBar",
  data: () => ({
    hideSlidersOn: ["CodeofConduct","Profile","SpeakerPage","scheduleDetails","PublicProfile","Contest"],
    userFound: false,
    userInfo: {},
    loadingUser: true
  }),
    ...
    shareMe() {
      if (navigator.share) {
        ...
        navigator
          ...
          .then(() => {
            console.log("Děkujeme za sdílení");
          })
          .catch((e) => {
            console.log(e);
          });

...
let DevProfilerServices = {
    ...
    **/
    updateUserProfileInfo: (uid, updatedData) => new Promise((resolve, reject) => {
        try {
            firebase.firestore.collection(CollectionName).doc(uid).update(updatedData)
            resolve({
                success: true
            })
        } catch (e) {
    ...
    **/
    ...
    findBadgeExists: (badgeCode) => new Promise(async (resolve, reject) => {
        try {
            let result = await firebase.firestore.collection('badges').where('code', '==', badgeCode).where('status','==',true).limit(1).get();
            // console.log(result)
            resolve({
    ...
    **/
    getAllUserBadges: (uid) => new Promise(async (resolve, reject) => {
        try {
            ...
            AllBadgesData = AllBadgesData.filter(badge=> badge.visible)
